...
| Modul | Deskripsi |
|---|---|
| **The Sentinel** (`/`) | Agen otonom bertenaga antrean BullMQ yang memindai anomali pasar dan metrik perilaku setiap 6 jam, memberikan panel deviasi, serta sistem peringatan instan (watchlist → notifikasi + webhook). |
| **The Multiverse** (`/multiverse`) | Visualisasi rantai keputusan kausal berbasis React Flow; melacak keputusan masa lalu dan menambah cabang simulasi (counter-factual branches). |
| **The Edge** (`/edge`) | Sandbox simulasi khusus Pro yang menjalankan 1.000 jalur probabilitas secara langsung untuk menguji variabel — termasuk variabel volatilitas. |
| **AI Content Studio** (`/studio`) | Killer feature: Vercel AI SDK + OpenAI menghasilkan analisis streaming via templat (Thread Analyst, TL;DR) dengan rantai sitasi terjaga. |
